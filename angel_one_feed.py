    ...
    def get_historical_candles(self, symbol: str, minutes_back: int = 375, retry: bool = True) -> pd.DataFrame:
        ...
        try:
            ...
            if not resp or not resp.get("status"):
                if retry:
                    log.warning(f"[ANGEL] getCandleData failed for {symbol}, retrying once in 3s: {resp}")
                    time.sleep(3)
                    return self.get_historical_candles(symbol, minutes_back, retry=False)
                log.error(f"[ANGEL] getCandleData failed for {symbol} after retry: {resp}")
                return pd.DataFrame()
            ...
            if retry:
                log.warning(f"[ANGEL] Exception fetching candles for {symbol}, retrying once in 3s: {e}")
                time.sleep(3)
                return self.get_historical_candles(symbol, minutes_back, retry=False)
            log.error(f"[ANGEL] Exception fetching candles for {symbol} after retry: {e}")
            return pd.DataFrame()
