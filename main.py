...
main.py
-------
Standalone entry point for ONE bot (see config.BOT_NAME). Deploy this whole
folder as its own Render Web Service / its own repo - it shares no state,
database, or process with the other two bots.
...
def job_morning_reset():
    cooldown.reset_all()
    telegram_notify.send_health_check(
        config.TELEGRAM_TOKEN, config.TELEGRAM_CHAT_ID, config.BOT_NAME, len(config.SYMBOLS)
    )
...
def bootstrap():
    ...
    feed.login()
    feed.load_instrument_master(config.SYMBOLS)
    feed.start_websocket(config.SYMBOLS)
...
@app.route("/trigger")
def manual_trigger():
    """Add ?force=true to bypass the market-hours check for testing."""
...
def telegram_test():
    """
    Sends a plain test message AND returns Telegram's raw API response, so
    failures show the actual reason (e.g. "chat not found", "bot was
    blocked by the user", "Unauthorized") instead of just true/false.
    """
    import requests as _requests
    if not config.TELEGRAM_TOKEN or not config.TELEGRAM_CHAT_ID:
        return jsonify({
            "sent": False,
            "reason": "TELEGRAM_TOKEN or TELEGRAM_CHAT_ID env var not set on Render",
        }), 400
    ...
    return jsonify({
        ...
        "token_prefix_used": config.TELEGRAM_TOKEN[:10] + "..." if config.TELEGRAM_TOKEN else None,
    })
